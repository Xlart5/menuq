...
function Burger({ double = false }: { double?: boolean }) {
  return (
    <group>
      {/* pan inferior */}
      <mesh position={[0, 0.0, 0]} castShadow>
        <cylinderGeometry args={[0.55, 0.46, 0.24, 48]} />
        <meshStandardMaterial {...M.bun} />
      </mesh>
      {/* patty 1 */}
      <mesh position={[0, 0.22, 0]} castShadow>
        <cylinderGeometry args={[0.58, 0.58, 0.16, 48]} />
        <meshStandardMaterial {...M.patty} />
      </mesh>
      {/* queso */}
      <mesh position={[0, 0.33, 0]} rotation={[0, 0.25, 0]} castShadow>
        <boxGeometry args={[1.06, 0.045, 1.06]} />
        <meshStandardMaterial {...M.cheese} />
      </mesh>
      {double && (
        <>
          ...
          <mesh position={[0, 0.55, 0]} rotation={[0, -0.3, 0]} castShadow>
            <boxGeometry args={[1.02, 0.045, 1.02]} />
            <meshStandardMaterial {...M.cheese} />
          </mesh>
        </>
      )}
      {/* lechuga */}
      <mesh position={[0, 0.47, 0]} scale={[1.22, 0.32, 1.22]} castShadow>
        <sphereGeometry args={[0.5, 48, 24]} />
        <meshStandardMaterial {...M.lettuce} />
      </mesh>
      {/* tomate */}
      <mesh position={[0, 0.53, 0]} castShadow>
        <cylinderGeometry args={[0.52, 0.52, 0.06, 48]} />
        <meshStandardMaterial {...M.tomato} />
      </mesh>
      {/* pan superior */}
      <mesh position={[0, 0.75, 0]} scale={[1, 0.72, 1]} castShadow>
        <sphereGeometry args={[0.58, 48, 24]} />
        <meshStandardMaterial {...M.bunTop} />
      </mesh>
      {/* sésamo */}
      {[
        [{ x: -0.22, y: 1.02, z: 0.1 }],
        [{ x: 0.18, y: 1.05, z: -0.12 }],
        [{ x: 0.05, y: 1.12, z: 0.18 }],
        [{ x: -0.12, y: 1.1, z: -0.2 }],
        [{ x: 0.28, y: 1.0, z: 0.22 }],
      ].flat().map((p, i) => (
...
function Pizza({ pepperoni = false }: { pepperoni?: boolean }) {
  const toppings = [
    [0.55, 0.12, 0.1],
    [0.2, 0.14, 0.55],
    [-0.45, 0.12, 0.4],
  ...
  return (
    <group rotation={[Math.PI / 2.4, 0, 0.2]}>
      {/* base */}
      <mesh castShadow>
        <cylinderGeometry args={[0.92, 0.78, 0.09, 56]} />
        <meshStandardMaterial {...M.crust} />
      </mesh>
      {/* salsa */}
      <mesh position={[0, 0.052, 0]}>
        <cylinderGeometry args={[0.82, 0.82, 0.02, 56]} />
        <meshStandardMaterial {...M.sauceRed} />
      </mesh>
      {/* queso fundido */}
      <mesh position={[0, 0.075, 0]}>
        <cylinderGeometry args={[0.78, 0.78, 0.02, 56]} />
        <meshStandardMaterial {...M.cheeseTop} />
      </mesh>
      {pepperoni
        ? toppings.map((p, i) => (
            <mesh key={i} position={[p[0] * 1.05, 0.1, p[2] * 1.05]} castShadow>
              <cylinderGeometry args={[0.1, 0.1, 0.03, 24]} />
              <meshStandardMaterial {...M.pepperoni} />
            </mesh>
          ))
        : toppings.slice(0, 4).map((p, i) => (
            <mesh key={i} position={[p[0], 0.1, p[2]]} scale={[0.75, 0.55, 0.75]} castShadow>
              <sphereGeometry args={[0.12, 16, 12]} />
              <meshStandardMaterial {...M.basil} />
            </mesh>
          ))}
    </group>
  );
